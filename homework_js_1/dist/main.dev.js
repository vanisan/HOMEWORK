...
function countAge() {
  var age = document.getElementById("one").value;
...
function symbolsGet() {
  var key = prompt("Choose and enter a number from 0 to 9");
...
function checkNum() {
  var chknum = prompt('Type the number between 100 to 999 (icludingly)');

  if (chknum[0] == chknum[1] || chknum[0] == chknum[2] || chknum[1] == chknum[2]) {
    alert('same chars FOUND');
  } else alert('same chars NOT FOUND');
}
/*TASK 4*/
...
function palindrome() {
  var numinp = prompt('Enter a name or value');
  if (numinp === numinp.split('').reverse().join('')) alert('Your number is palindrome!');else alert('It is not palindrome');
}